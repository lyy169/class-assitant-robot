...
def build_teacher_reports_html(current_user: Optional[dict] = None, result_id: Optional[str] = None) -> str:
    ...
    <script>
      ...
      async function loadDetail(id){
        ...
        document.getElementById("app").innerHTML=`<section class="grid"><div class="metric"><span>综合评分</span><strong>${text(sc.overall_score,0)}</strong></div><div class="metric"><span>专注度</span><strong>${text(sc.attention_score,0)}</strong></div><div class="metric"><span>活跃度</span><strong>${text(sc.activity_score,0)}</strong></div><div class="metric"><span>风险等级</span><strong>${riskLabel(report.risk_level)}</strong></div></section><section class="card"><h2>课堂基本信息</h2><p>${text(b.classroom_name)} · ${text(b.teacher_name)} · ${text(b.created_at)} · ${sourceLabel(report.dataset_source)}</p><a class="button secondary" href="${report.dashboard_url}">打开原始课堂分析</a></section><section class="grid"><div class="chart-panel"><h2>数据依据：专注 / 活跃曲线</h2><div id="curve" class="chart"></div></div><div class="chart-panel"><h2>教学阶段结构</h2><div id="stage" class="chart"></div></div></section><section class="grid"><div class="card"><h2>课堂结论</h2>${(report.highlights||[]).map(x=>`<div class="record">${x}</div>`).join("")}<h2>主要风险</h2>${(report.risks||[]).map(x=>`<div class="record">${x}</div>`).join("")||"<div class='empty'>暂无明显风险。</div>"}</div><div class="insight-panel"><h2>改进建议</h2>${(report.recommendations||[]).map(x=>`<div class="record">${x}</div>`).join("")}</div></section>${enhancedIssuesSection(enhancedIssues)}<section class="card"><h2>提问与响应分析</h2><p>提问数：${text(qa.question_count,0)} · 响应率：${text(qa.response_rate,0)}</p><div class="list">${(qa.events||[]).map(e=>`<div class="record">${text(e.text)}<br><span class="muted">${text(e.start_sec)}s</span></div>`).join("")||"<div class='empty'>暂无提问事件。</div>"}</div></section><section class="card"><h2>AI 综合评语</h2><p id="ai-status" class="muted">AI 综合评语未启用时，当前展示规则报告。</p><div id="ai-content" class="record">${text((report.ai_summary||{}).content,"未配置或尚未生成。")}</div><button class="button" id="ai-button">生成 AI 综合评语</button></section>`;
